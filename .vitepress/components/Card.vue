  ...
  <div class="info-card">
    <div class="info-card__header">
      <div class="info-card__image-wrapper">
        <img class="info-card__image" :src="item.image" alt="Image" >
	  </div>
      <div class="info-card__header-right">
        <span class="info-card__category">{{ item.category }}</span>
        <div class="info-card__icons">
		  <a class="info-card__icon-twitter" :href="item.twitter" target="_blank">
		  ...
		  <a class="info-card__icon-globe" :href="item.globe" target="_blank">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke-width="2">
    ...
    <div class="info-card__content">
      <div class="info-card__logo-wrapper">
        <img class="info-card__logo" :src="item.logo" alt="Logo" width="64" height="64"/>
      </div>
      <div class="info-card__title">{{ item.title }}</div>
      <p class="info-card__description">{{ item.description }}</p>
      <div class="info-card__tags">
        <span v-for="(tag, index) in item.tags" :key="index" class="info-card__tag">
          {{ tag }}
        </span>
      </div>
    </div>
