...
export const createEvent = async (
  ...
  try {
    ...
    const requestBody = {
      ...
      location:
        eventParams.event_mode === '线下活动'
          ? eventParams.location.trim()
          : '',
      link:
        eventParams.event_mode === '线上活动' ? eventParams.link.trim() : '',
      start_time: eventParams.start_time,
      end_time: eventParams.end_time || '',
      cover_img: eventParams.cover_img || '',
      tags: eventParams.tags || [],
      twitter: eventParams.twitter || '',
      // 可选参数
      ...(eventParams.max_participants && {
        max_participants: eventParams.max_participants,
      }),
      ...(eventParams.registration_deadline && {
        registration_deadline: eventParams.registration_deadline,
      }),
      ...(eventParams.require_approval !== undefined && {
        require_approval: eventParams.require_approval,
      }),
      ...(eventParams.allow_waitlist !== undefined && {
        allow_waitlist: eventParams.allow_waitlist,
      }),
    };

    console.log('创建活动请求参数:', requestBody);
    ...
    return {
      success: false,
      message: response.message || '活动创建失败',
    };
  } catch (error: any) {
...
export const getEvents = async (
  ...
  try {
    ...
    return {
      success: false,
      message: response.message || '获取活动列表失败',
    };
  } catch (error: any) {
...
export const getEventById = async (eventId: string): Promise<EventResult> => {
  try {
    ...
    return {
      success: false,
      message: response.message || '获取活动失败',
    };
  } catch (error: any) {
